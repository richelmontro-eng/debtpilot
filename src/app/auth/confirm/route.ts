...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!await verifyEmailToken(supabase.auth, tokenHash, type)) return redirect(request, '/auth/error');

    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) return redirect(request, '/auth/error');
    ...
    const { data: profile, error: profileError } = await supabase
      ...
      .eq('user_id', user.id)
      .maybeSingle();
    if (profileError) return redirect(request, '/auth/error');
