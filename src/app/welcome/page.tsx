...
export default function WelcomePage() {
  ...
  return <main className="min-h-screen bg-slate-950 px-5 py-10 text-slate-100"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between"><p className="text-sm text-cyan-300">Step {step} of 5</p><p className="text-sm text-slate-500">{steps[step - 1]}</p></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-800"><div className="h-full bg-cyan-400 transition-all" style={{ width: `${step * 20}%` }}/></div><section className="mt-8 rounded-3xl border border-slate-800 bg-slate-900 p-6"><StepContent step={step} draft={draft} update={update}/></section>{message && <p className="mt-4 text-sm text-rose-300">{message}</p>}<div className="mt-6 flex justify-between"><button onClick={() => step > 1 ? setStep(step - 1) : setStarted(false)} className="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-3 text-slate-300"><ChevronLeft size={17}/>Back</button><button onClick={saveAndContinue} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-5 py-3 font-semibold text-slate-950 disabled:opacity-60">{step === 5 ? 'Finish Setup' : 'Continue'}<ChevronRight size={17}/></button></div></div></main>;
}

function StepContent({ step, draft, update }: { step: number; draft: Draft; update: <K extends keyof Draft>(key: K, value: Draft[K]) => void }) {
  if (step === 1) return <div><h2 className="text-2xl font-semibold">Income & Pay Schedule</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Text label="Display Name" value={draft.displayName} onChange={value => update('displayName', value)}/><label className="text-xs text-slate-400">Pay Frequency<select className="field mt-1 w-full" value={draft.payFrequency} onChange={e => update('payFrequency', e.target.value as PayFrequency)}><option value="weekly">Weekly</option><option value="biweekly">Every 2 weeks</option><option value="semimonthly">Twice monthly</option><option value="monthly">Monthly</option></select></label><NumberField label="Net Pay" value={draft.netPay} onChange={value => update('netPay', value)}/></div></div>;
  if (step === 2) return <div><h2 className="text-2xl font-semibold">Accounts</h2><div className="mt-5 grid gap-4 sm:grid-cols-3"><NumberField label="Checking" value={draft.checking} onChange={value => update('checking', value)}/><NumberField label="Savings" value={draft.savings} onChange={value => update('savings', value)}/><NumberField label="Protected Checking Cushion" value={draft.cushion} onChange={value => update('cushion', value)}/></div></div>;
  if (step === 3) return <Collection title="Debts" onAdd={() => update('debts', [...draft.debts, { id: crypto.randomUUID(), name: 'New debt', balance: 0, apr: 0, minimum: 0 }])}>{draft.debts.map(item => <div key={item.id} className="grid gap-3 rounded-xl border border-slate-800 p-4 sm:grid-cols-4"><Text label="Name" value={item.name} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, name: value } : row))}/><NumberField label="Balance" value={item.balance} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, balance: value } : row))}/><NumberField label="APR" value={item.apr} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, apr: value } : row))}/><Remove onClick={() => update('debts', draft.debts.filter(row => row.id !== item.id))}/></div>)}</Collection>;
  if (step === 4) return <Collection title="Bills" onAdd={() => update('bills', [...draft.bills, { id: crypto.randomUUID(), name: 'New bill', amount: 0, dueDay: 1, frequency: 'monthly' }])}>{draft.bills.map(item => <div key={item.id} className="grid gap-3 rounded-xl border border-slate-800 p-4 sm:grid-cols-4"><Text label="Name" value={item.name} onChange={value => update('bills', draft.bills.map(row => row.id === item.id ? { ...row, name: value } : row))}/><NumberField label="Amount" value={item.amount} onChange={value => update('bills', draft.bills.map(row => row.id === item.id ? { ...row, amount: value } : row))}/><NumberField label="Due Day" value={item.dueDay} onChange={value => update('bills', draft.bills.map(row => row.id === item.id ? { ...row, dueDay: value } : row))}/><Remove onClick={() => update('bills', draft.bills.filter(row => row.id !== item.id))}/></div>)}</Collection>;
  return <Collection title="Financial Goals" onAdd={() => update('goals', [...draft.goals, { id: crypto.randomUUID(), name: 'New goal', goalType: 'custom', targetAmount: 0, currentAmount: 0, priority: 2 }])}>{draft.goals.map(item => <div key={item.id} className="grid gap-3 rounded-xl border border-slate-800 p-4 sm:grid-cols-4"><Text label="Name" value={item.name} onChange={value => update('goals', draft.goals.map(row => row.id === item.id ? { ...row, name: value } : row))}/><NumberField label="Target" value={item.targetAmount} onChange={value => update('goals', draft.goals.map(row => row.id === item.id ? { ...row, targetAmount: value } : row))}/><NumberField label="Current" value={item.currentAmount} onChange={value => update('goals', draft.goals.map(row => row.id === item.id ? { ...row, currentAmount: value } : row))}/><Remove onClick={() => update('goals', draft.goals.filter(row => row.id !== item.id))}/></div>)}</Collection>;
}
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="text-xs text-slate-400">{label}<input type="number" min="0" className="field mt-1 w-full" value={value} onChange={e => onChange(Number(e.target.value))}/></label>; }
