...
function StepContent({ step, draft, update }: { step: number; draft: Draft; update: <K extends keyof Draft>(key: K, value: Draft[K]) => void }) {
  ...
  if (step === 3) return <Collection title="Debts" onAdd={() => update('debts', [...draft.debts, { id: crypto.randomUUID(), name: 'New debt', balance: 0, apr: 0, minimum: 0, promotionType: 'none', promotionalApr: 0, promotionEndDate: '', postPromotionApr: 0, originalPromotionalBalance: 0, estimatedDeferredInterest: 0 }])}>{draft.debts.map(item => <div key={item.id} className="rounded-xl border border-slate-800 p-4"><div className="grid gap-3 sm:grid-cols-5"><Text label="Name" value={item.name} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, name: value } : row))}/><NumberField label="Balance" value={item.balance} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, balance: value } : row))}/><NumberField label={<InfoTooltip label="Regular APR">The annual percentage rate that applies outside any promotional period. DebtPilot uses the promotional rate until the promotion ends.</InfoTooltip>} value={item.apr} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, apr: value } : row))}/><NumberField label={<InfoTooltip label="Minimum payment">The smallest payment the lender requires each month. Missing it can trigger fees or delinquency. DebtPilot reserves minimum payments before recommending any extra payment.</InfoTooltip>} value={item.minimum} onChange={value => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, minimum: value } : row))}/><Remove onClick={() => update('debts', draft.debts.filter(row => row.id !== item.id))}/></div><WelcomePromotionFields item={item} updateItem={changes => update('debts', draft.debts.map(row => row.id === item.id ? { ...row, ...changes } : row))}/></div>)}</Collection>;
