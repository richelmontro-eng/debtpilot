...
import { Activity, ArrowRight, Bot, CalendarDays, CircleDollarSign, CreditCard, Plus, ReceiptText, Sparkles, Target, TrendingUp, WalletCards } from 'lucide-react';
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100"><div className="mx-auto max-w-7xl px-5 py-8">
    <header className="mb-8">
      <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-sm text-cyan-300"><Sparkles size={16}/> Financial command center</div>
      <h1 className="text-4xl font-semibold">{greeting}{profile.display_name ? `, ${profile.display_name}` : ''}.</h1>
      <p className="mt-2 text-slate-500">{dateLabel}</p>
      <p className="mt-3 max-w-3xl text-slate-400">Here is how you are doing, what changed, and the strongest next move.</p>
    </header>
    ...
    <section className={`rounded-3xl border p-6 ${pulseStyle}`}>
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div><p className="text-xs font-semibold uppercase tracking-[0.2em]">Financial pulse</p><h2 className="mt-2 text-4xl font-semibold">{pulse}</h2><p className="mt-3 max-w-xl text-sm leading-6 opacity-80">{pulse === 'Strong' ? 'Your essentials and safety buffer are covered, with room to make progress.' : pulse === 'Stable' ? 'Your plan is balanced. Stay focused on the recommended next action.' : 'Protect required expenses and rebuild your cash buffer before optional moves.'}</p></div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><PulseStat label="Health" value={`${health}/100`}/><PulseStat label="Net worth" value={money.format(netWorth)}/><PulseStat label="Checking" value={money.format(checking)}/><PulseStat label="Safe extra" value={money.format(safeExtra)}/></div>
      </div>
    </section>

    <section className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <Card title="Today’s priorities">
        <Priority tone={checking < cushion ? 'high' : 'good'} title={checking < cushion ? 'Restore your checking cushion' : 'Checking cushion protected'} detail={checking < cushion ? `${money.format(cushion - checking)} is needed to restore your preferred buffer.` : `${money.format(Math.max(0, checking - cushion))} remains above your protected minimum.`}/>
        ...
        {topGoal && <Priority tone="good" title={`${topGoal.name} is ${Math.round(topGoal.current / Math.max(1, topGoal.target) * 100)}% complete`} detail={`${money.format(topGoal.target - topGoal.current)} remains.`}/>} 
      </Card>
      <div className="rounded-3xl border border-cyan-400/30 bg-cyan-400/10 p-6"><div className="flex items-center gap-2 text-cyan-300"><Bot size={18}/><p className="text-xs font-semibold uppercase tracking-[0.2em]">Recommended today</p></div><h2 className="mt-4 text-3xl font-semibold">{recommendation.title}</h2><p className="mt-4 text-sm leading-6 text-slate-300">{recommendation.reason}</p><div className="mt-5 flex items-center justify-between rounded-2xl border border-cyan-400/20 bg-slate-950/30 p-4"><div><p className="text-xs text-cyan-300">Confidence</p><p className="mt-1 text-2xl font-semibold">{recommendation.confidence}%</p></div><Link href="/pilot" className="inline-flex items-center gap-2 rounded-xl border border-cyan-400/30 px-4 py-2 text-sm text-cyan-200">Why this? <ArrowRight size={16}/></Link></div></div>
    </section>
