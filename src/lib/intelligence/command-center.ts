...
export function buildCommandCenter(input: {
  ...
  const events = deriveFinancialEvents({ now: input.now, cycleDays: input.cycleDays, payPerCheck: input.financialState.payPerCheck, bills: input.bills, debts: input.debts, goals: input.goals, pulse: pilot.pulse, recommendation: pilot.recommendation, recommendationHistory: input.recommendationHistory });
  return {
    pilot,
    ...
    insights: generatePilotInsights({ checking: input.checking, checkingCushion: input.checkingCushion, safeExtra: input.financialState.safeExtra, billsReserve: input.billsReserve, payPerCheck: input.financialState.payPerCheck, debts: input.debts, bills: input.bills, goals: input.goals, recommendation: pilot.recommendation, events, snapshots: input.snapshots }),
